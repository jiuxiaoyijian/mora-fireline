...
<div id="game-stage">
  <main id="game-world" aria-label="火线街区游戏">
    ...
    <div class="hud-top">
      <div class="town-label"><span class="eyebrow">FIRELINE / 街区 01</span><h1>松风镇</h1><span id="phase-badge">准备阶段</span></div>
    ...
    <div class="map-toolbar"><span class="live-label"><i id="state-dot"></i><b id="map-status">规划家园</b></span><div class="map-toggles"><button id="coverage" aria-pressed="false">喷淋覆盖</button><button id="routes" aria-pressed="false">传播路径</button><button id="grid-open">键盘网格</button></div></div>
    <div class="insight" id="insight"><span class="insight-number">01</span><div><strong>先为家园添一道防线。</strong><p>在西侧草地放一格防火带；可撤销。孤立一格不保证挡住山火。</p></div></div>
    <div id="warning-overlay" class="warning-overlay" hidden><span class="eyebrow">居民已转移 · 守住大家的家园</span><strong>山火即将抵达</strong><p>风向：西 → 东</p><b id="warning-count">3</b></div>
  ...
  <section class="report" id="report" hidden role="dialog" aria-modal="true" aria-labelledby="report-title" tabindex="-1"><button id="report-close" class="dialog-close" aria-label="关闭复盘">×</button><div class="report-head"><div><div class="eyebrow">AFTER THE FIRE / 家园回望</div><h2 id="report-title"></h2><p id="comparison"></p></div><div class="report-score" id="report-score"></div></div><div class="report-body"><div><h3>这次发生了什么</h3><p id="report-reason"></p><ol id="event-list"></ol></div><div class="report-next"><h3>再守一次家园</h3><p>调整一处布局，再用相同火情验证你的判断。</p><div><button id="edit-again" class="primary">调整布局 →</button><button id="retry" class="secondary">原样重试</button></div></div></div></section>
  <div class="toast" id="toast" role="status" aria-live="polite" hidden></div>
  <dialog id="help" aria-labelledby="help-heading"><form method="dialog"><button class="dialog-close" aria-label="关闭玩法说明">×</button></form><div class="eyebrow">FIELD GUIDE / 玩法说明</div><h2 id="help-heading">在山火来临之前，守住家园。</h2><ol class="help-steps"><li><b>先建一道防线。</b>用防火带隔断传播，用蓄水喷淋预防起火。安置好 12 栋住宅后开始，山火自西向东进入。</li><li><b>看懂防线。</b>空地有可燃植被，不能代替防火带。喷淋在两格步行距离内共享降温能力，无法救回已燃烧的住宅。</li><li><b>观察与调整。</b>模拟可暂停、加速或直接查看结果；根据实际传播记录调整，再试一次。</li><li><b>守住家园。</b>保住至少 8 栋即达标。同样布局与火情，结果完全相同。</li></ol><p class="help-tip">预算 12 点 · 防火带 1 点 / 格 · 喷淋 4 点 / 座<br>1–4 切换工具 · 空格开始 / 暂停 · Ctrl/Cmd+Z 撤销 · Esc 取消搬迁 / 打开菜单<br>失焦自动暂停；布局保存于本浏览器，撤销历史与模拟进程不跨刷新保存。</p><button id="help-close" class="primary">明白了 →</button></dialog>
  <dialog id="title-screen" class="title-card" aria-labelledby="title-heading"><div class="eyebrow">FIRELINE / 林缘小镇的火险季</div><h2 id="title-heading">火线街区</h2><p class="title-tagline">让风过去，<br>让家留下。</p><p class="title-story">山火将至，你是松风镇的社区规划者。<br>为 12 户家庭，筑起家园的防线。</p><div class="title-facts"><span>建造防线</span><i>·</i><span>观察火势</span><i>·</i><span>再试一次</span></div><div class="title-actions"><button id="enter-town" class="primary">进入小镇 →</button><button id="title-new" class="secondary">重新规划</button><div><button id="title-help" class="quiet">玩法说明</button><button id="title-sound" class="quiet" aria-pressed="false">声音：关</button></div></div><small id="continue-note">自由准备 · 45 秒火情 · 可加速</small></dialog>
  <dialog id="pause-menu" class="pause-card" aria-labelledby="pause-heading"><div class="eyebrow">松风镇 / 暂歇片刻</div><h2 id="pause-heading">家园会等你回来。</h2><p>当前布局已保留；运行中的火情暂停。</p><div class="menu-stack"><button id="pause-close" class="primary">返回游戏 →</button><button id="pause-restart" class="secondary">返回建造，调整布局</button><button id="pause-title" class="quiet">返回主菜单</button></div></dialog>
  <dialog id="grid-dialog" class="grid-dialog" aria-labelledby="grid-heading"><button id="grid-close" class="dialog-close" aria-label="关闭键盘网格">×</button><div class="eyebrow">KEYBOARD / 键盘建造</div><h2 id="grid-heading">街区地图</h2><p>数字 1–4 选择工具，Tab 选择格子，Enter 建造。与三维地图实时同步。</p><div id="grid" class="a11y-grid" role="group" aria-label="街区建造网格"></div></dialog>
