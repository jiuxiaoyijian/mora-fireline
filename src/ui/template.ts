...
<div id="game-stage">
  <main id="game-world" aria-label="火线街区游戏">
    ...
    <div class="wind-label"><strong id="wind-current"></strong><small id="weather-plan"></small></div><aside class="lesson-card"><strong id="lesson-title"></strong><p id="lesson-text"></p><div><button id="hint-open">巡护提示</button><small id="lesson-progress">先读委托，观察火源，再布置防线。</small></div><p id="lesson-hint" hidden></p></aside><div id="water-pressure" class="water-pressure" aria-label="喷淋拦截储备" hidden></div><div id="fire-feedback" class="fire-feedback" role="status">橙线：地面受热 · 绿线：被阻断 · 青圈：当前有水压</div>
  ...
  <dialog id="outcome" class="outcome-card" aria-labelledby="outcome-heading"><div class="eyebrow">松风镇 · 本次演练结果</div><h2 id="outcome-heading"></h2><div id="home-lights" class="home-lights" aria-hidden="true"></div><p id="outcome-summary"></p><p id="outcome-mission" class="mission-verdict"></p><p id="outcome-completion" hidden></p><p id="outcome-tip"></p><section id="campaign-ending" hidden><p>把这次守护留作纪念，邀请朋友设计他们的防线。</p><button id="share-ending" class="primary">复制通关分享</button><button id="restart-campaign" class="secondary">重新开始八次值守</button><p id="share-status" role="status"></p><textarea id="share-text" aria-label="通关分享文字" readonly hidden></textarea></section><div class="outcome-actions"><button id="next-level" class="primary" hidden>前往下一关 →</button><button id="outcome-edit" class="primary">保留布局，改善防线 →</button><button id="outcome-details" class="secondary">看看火从哪里来</button></div></dialog>
<dialog id="level-select" aria-labelledby="levels-heading"><button id="restore-campaign" class="secondary">恢复重开前进度</button><button id="levels-close" class="dialog-close" aria-label="关闭关卡选择">×</button><span class="eyebrow">巡护路线 / 八次值守</span><h2 id="levels-heading">沿着山海，守住家园。</h2><p>通关后解锁下一次委托。已完成的关卡可回访，尝试用更少预算完成保护。</p><div id="level-list"></div></dialog>
</div>`;
